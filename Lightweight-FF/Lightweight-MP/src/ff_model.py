...
class FF_model(torch.nn.Module):
    ...
    def forward_downstream_multi_pass_one_by_one(
        ...
        if index <self.opt.model.num_layers-1:
            if ssq_all.data[0,labels["class_labels"]] < scalar_outputs[f"pos_mean_layer{index}"] - scalar_outputs[f"pos_std_layer{index}"]:
                return 'contine'
            else:

                with torch.no_grad():
                    prediction = torch.argmax(ssq_all.data)
                    return (prediction == labels["class_labels"])
        if index ==self.opt.model.num_layers-1:
                with torch.no_grad():
                    prediction = torch.argmax(ssq_all.data)
                    return (prediction == labels["class_labels"])
    ...
    def forward_downstream_multi_pass_meanstd(
        self, inputs, labels, scalar_outputs=None,index =-1,
    ):
        if scalar_outputs is None:
            scalar_outputs = {
        ...
        for sample_index,label in enumerate(labels["class_labels"]):
            mean_all.append(ssq_all.data[sample_index,label])
    ...
    def forward_downstream_multi_pass(
        ...
        classification_accuracy = utils.get_accuracy_mp(
