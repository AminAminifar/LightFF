...
def validate_or_test_meanstd(opt, model):
    ...
    with torch.no_grad():
        ...
        for inputs, labels in data_loader:
            ...
            for i in range(opt.model.num_layers):
                scalar_outputs = model.forward_downstream_classification_meanstd(
                    inputs, labels, scalar_outputs=scalar_outputs,index=i
                )
...
def test_one_by_one(opt, model,outputs):
    ...
    layer_frequency=np.zeros(opt.model.num_layers)
    ...
    with torch.no_grad():
        for inputs, labels in data_loader:
            ...
            for i in range(opt.model.num_layers):
                layers_count+=1
                layer_individual +=1
                feedback = model.forward_downstream_classification_one_by_one(
            ...
            layer_frequency[layer_individual-1]+=1
    ...
    print('layer_frequency')
    print(layer_frequency/num_steps_per_epoch)
