...
def calculate_goodness_distributions(matrix, y_predicted_on_layer, targets):  # matrix is softmax_output_on_layer
    ...
    # mean_all_incorrect_labels /= classes * (classes - 1)
    # std_all_incorrect_labels /= classes * (classes - 1)
...
# print('train first-layer softmax')

# one_pass_1_layer = One_Pass(1024,10)
...
#         one_pass_1_layer.train(layers_act[0],target)


#     print('Testing for one_pass_1')
#     correct = 0
#     total = 0
#     # since we're not training, we don't need to calculate the gradients for our outputs
#     with torch.no_grad():
...
#             _, one_pass_1_output = one_pass_1_layer(layers_act[0])
...
# print('train second-layer softmax')
# one_pass_2_layer = One_Pass(1024,10)
...
#         one_pass_2_layer.train(layers_act[1],target)

#     print('Testing for one_pass_2')
#     correct = 0
#     total = 0
#     # since we're not training, we don't need to calculate the gradients for our outputs
#     with torch.no_grad():
...
#             _, one_pass_2_output = one_pass_2_layer(layers_act[1])

#             # the class with the highest energy is what we choose as prediction

#             _, predicted = torch.max(one_pass_2_output.data, 1)
