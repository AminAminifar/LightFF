...
X_train = torch.tensor(X_train).to(torch.float32) #77554
Y_train = torch.tensor(Y_train).to(torch.int64) #10000
X_test  = torch.tensor(X_test).to(torch.float32)#21892
...
valloader = torch.utils.data.DataLoader(valset, batch_size=valid_size,
                                          shuffle=True)

testloader = torch.utils.data.DataLoader(testset, batch_size=21892,
                                         shuffle=False)
...
print("percentage for layers_up_to ", values, " : ", counts/21892)
