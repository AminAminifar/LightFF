...
def each_patient_detection(which_patients,Train_flag,layers,length_network):
    ...
    if Train_flag:
        ...
        model = Train.build_model(x_pos=x_pos, x_neg=x_neg,layers = layers,id=which_patients)

    name = 'temp'
    model = torch.load(os.path.split(os.path.realpath(__file__))[0]+'/model/' + name+which_patients)

    train_error = Evaluation.eval_train_set(model, inputs=X_train, targets=Y_train)
    test_error = Evaluation.eval_test_set(model, inputs=X_test, targets=Y_test) 
    val_error = Evaluation.eval_val_set(model, inputs=X_val, targets=Y_val,size = val_size )#val_size)

    mean,std = tools.analysis_val_set(model, inputs=X_val, targets=Y_val,size = val_size,length_network = length_network) # measure each layer's error/ based on the whole sample
    test_error_light, light_number_layers,percentage = Evaluation.eval_val_set_light(model, inputs=X_test, targets=Y_test,means = mean, stds = std,size=test_size,length_network= length_network) # measure how many layer needed/ based on each sample
