...
class FF_CHBMIT(torch.utils.data.Dataset):
    ...
    def _get_pos_sample(self, sample, class_label):
        ...
        pos_sample = sample.clone()
        pos_sample[0, 0, : self.num_classes] = one_hot_label #caution: need to change based on the dataset
        return pos_sample
    ...
    def _get_neg_sample(self, sample, class_label):
        ...
        neg_sample = sample.clone()
        neg_sample[0, 0, : self.num_classes] = one_hot_label #caution: need to change based on the dataset
        return neg_sample
    ...
    def _get_all_sample(self, sample):
        ...
        for i in range(self.num_classes):
            ...
            one_hot_label = torch.nn.functional.one_hot(
            torch.tensor(i), num_classes=self.num_classes)
            all_samples[i, 0, 0, : self.num_classes] = one_hot_label.clone() #caution: need to change based on the dataset
        return all_samples
