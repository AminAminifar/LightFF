...
def overlay_on_x_neutral(x):
    """Replace the first 5 pixels of data [x] with 0.1s
    """
    x_ = x.clone()
    x_[:, :5] *= 0.0
...
class Net(torch.nn.Module):
    def __init__(self, dims):
        ...
        for d in range(1, len(dims)):
            ...
            for i in range(1, d):
                in_dim += dims[i]
            self.softmax_layers += [SoftmaxLayer(in_features=in_dim, out_features=5)]  # .cuda()
    ...
    def light_predict_analysis(self, x, num_layers):  # dims is not needed; just num layers
        ...
        softmax_output_on_layer = np.zeros((num_layers, num_samples, 5))  # 5 is the number of softmax neurons
...
def build_model(x_pos, x_neg, x_neutral, targets,layers):
    ...
    for epoch in tqdm(range(Num_Epochs)):
        ...
        train_data_record_indices = range(0, 87554-10000-54)
        train_data_record_indices_shuffled = shuffle(train_data_record_indices)

        batch_size = 500
        num_batches = int((87554-10000-54) / batch_size)
        chunk_indices = np.array_split(train_data_record_indices_shuffled, num_batches)
        for i in range(num_batches):
            x_neutral_, targets_ = x_neutral[chunk_indices[i]], targets[chunk_indices[i]]
            model.train_softmax_layer(x_neutral_, targets_.to(torch.int64), batch_size, dims)  # , num_layers, num_neurons
